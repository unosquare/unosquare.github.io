...
(function (angular) {
    ...
    angular.module('tubular', ['tubular.core', 'tubular.directives', 'tubular.services', 'tubular.models']).info({ version: '1.8.2' });
})(angular);
...
(function (angular) {
    ...
    angular.module('tubular.directives').controller('tbGridController', ['$scope', 'tubularPopupService', 'tubularModel', '$http', 'tubularConfig', '$window', 'tubularLocalPager', 'modelSaver', 'compareOperators', 'sortDirection', function ($scope, tubularPopupService, TubularModel, $http, tubularConfig, $window, tubularLocalPager, modelSaver, compareOperators, sortDirection) {
        var $ctrl = this;
        ...
        $ctrl.retrieveLocalData = function (request) {
            if (angular.isArray($ctrl.gridDatasource)) {
                $ctrl.currentRequest = tubularLocalPager.process(request, $ctrl.gridDatasource).then($ctrl.processPayload, $ctrl.processError).then(function () {
        ...
        $ctrl.retrieveRemoteDataClientside = function (request) {
            $ctrl.currentRequest = $http(request).then(function (response) {
                return tubularLocalPager.process(request, response.data);
...
(function (angular) {
    ...
    .service('tubularHttp', ['$http', '$q', '$document', 'tubularConfig', '$window', function ($http, $q, $document, tubularConfig, $window) {
        ...
        var prefix = tubularConfig.localStorage.prefix();
        var me = this;
        var storage = tubularConfig.useSessionForAuthData() ? $window.sessionStorage : $window.localStorage;

        function init() {
            var savedData = angular.fromJson(storage.getItem(prefix + authData));
...
(function (angular) {
    ...
    .service('tubularTemplateService', ['$templateCache', 'translateFilter', 'dataTypes', 'compareOperators', 'sortDirection', function ($templateCache, translateFilter, dataTypes, compareOperators, sortDirection) {
        ...
        me.generateGridToExport = function (columns, options) {
            ...
            var rowsDefinitions = me.generateCellsToExport(columns, options.Mode);
